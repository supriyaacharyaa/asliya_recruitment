...
const footerLinks = {
  "Quick Links": [
    ...
    { label: "Mass Recruitment Campaigns", href: "/services/mass" },
  ...
  Services: [
    ...
    { label: "Campaigns", href: "/services/mass" },
  ],
  Resources: [
    { label: "Blog / Insights", href: "/blog" },
    { label: "FAQs", href: "/#" },
    { label: "Download Our Profile", href: "/#" },
    { label: "Service Guide", href: "/#" },
...
const contactItems = [
  ...
  {
    Icon: MapPin,
    text: " Muntaza Trading Center, Office 3, Floor 6, Building 1, Al Muntazah Hiteen St, Rawdat Al Khail St, Doha, Qatar",
...
const handleSubscribe = async (e) => {
  ...
  try {
    const { data } = await axios.post("/enquiry", {
      type: "newsletter",
      email,
    });
    ...
    <div className="w-full sm:w-auto sm:max-w-md">
      <form onSubmit={handleSubscribe} className="flex w-full gap-2">
        <input
          ...
          disabled={loading}
          className="flex-1 min-w-0 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 outline-none focus:border-blue-500/60 focus:ring-2 focus:ring-blue-500/20 transition-all duration-200 disabled:opacity-60"
        />
        <button
          type="submit"
          disabled={loading}
          className="shrink-0 flex items-center gap-2 bg-blue-700 hover:bg-blue-600 active:bg-blue-800 text-white text-sm font-bold px-4 sm:px-5 py-2.5 rounded-xl transition-all duration-200 whitespace-nowrap disabled:opacity-70"
        >
          {subscribed ? (
            "Done ✓"
          ) : loading ? (
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-5
        ...
        <div className="text-center sm:text-left">
          <p>© 2018–{new Date().getFullYear()} Asliya Recruitment W.L.L. All rights reserved.</p>
