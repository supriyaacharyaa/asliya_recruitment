...
// Forces a full page reload to the given URL, even if already on the same path,
// and scrolls to the hash section after reload.
function hardNavigate(url) {
  const target = new URL(url, window.location.origin);
  const isSamePath = target.pathname === window.location.pathname;

  if (isSamePath && target.hash) {
    // Save the hash so we can scroll to it after the reload
    sessionStorage.setItem("__scrollToHash", target.hash);
    // Set href first (updates the hash in the address bar), then reload
...
export default function Navbar() {
  ...
  // After a same-page reload, scroll to the saved hash target
  useEffect(() => {
