...
import { clientsData as partners } from "../mockData";
      ...
      <Container>
        ...
        >
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {partners.map((p, i) => {
              const [imgError, setImgError] = [p.__imgError, () => {}]; // placeholder, see PartnerChip below
              return (
                <PartnerChip key={p.name} partner={p} index={i} />
              );
            })}
          </div>
        </motion.div>
...
/* ── Individual partner chip (separate component so each logo
     can manage its own image-load-error fallback state) ── */
function PartnerChip({ partner: p, index: i }) {
  const [imgError, setImgError] = useState(false);
  const showLogo = p.logo && !imgError;
  ...
  return (
    <motion.div
      ...
      className="partner-chip flex items-center gap-3 bg-white rounded-2xl px-5 py-3.5 cursor-default"
      style={{ border:"1px solid rgba(21,72,149,0.10)", boxShadow:"0 2px 12px rgba(21,72,149,0.06)" }}
    >
      <div
        className="w-28 h-16 rounded-xl flex items-center justify-center flex-shrink-0 overflow-hidden"
        style={{
          background: showLogo ? "transparent" : `linear-gradient(135deg,${BLUE} 0%,#0d2f6b 100%)`,
        }}
      >
        {showLogo ? (
          <img
