            ...
            <>
              <div className="flex gap-2.5">
                ...
                <motion.button
                  ...
                  disabled={loading}
                  className="flex items-center gap-2 px-6 py-4 rounded-xl bg-white text-[#154895] text-sm font-bold hover:bg-gray-50 disabled:opacity-60 transition-colors flex-shrink-0 shadow-lg shadow-black/10"
                >
...
export default function BlogPage() {
  ...
  // FIX: Accept all params explicitly so the function never closes over stale state.
  const fetchBlogs = async ({ pg = 1, cat = "All", q = "" } = {}) => {
    setLoading(true);
    try {
      ...
      // Only rebuild category list on the unfiltered default fetch
      if (cat === "All" && !q) {
  ...
  // FIX: Pass current state values explicitly — no stale closure risk.
  useEffect(() => {
    fetchBlogs({ pg: page, cat: activeCategory, q: search });
  }, [page, activeCategory]);

  // FIX: Debounced search passes the latest search value directly.
  useEffect(() => {
    clearTimeout(searchTimeout.current);
    searchTimeout.current = setTimeout(() => {
      setPage(1);
      fetchBlogs({ pg: 1, cat: activeCategory, q: search });
    }, 350);
        ...
        <Container className="relative z-10">
          ...
          >
            {[
              ...
              >
                <Icon size={12} className="text-brand-secondary" />
                {label}
              </motion.div>
            ))}
          </motion.div>
