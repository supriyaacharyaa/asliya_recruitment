...
// Defined outside component — stable reference, no re-creation on every render
const TABS = [
...
export default function BlogAdmin() {
  ...
  // ── Single fetch function that reads its own args, no closure over state ──
  ...
  }, []); // no deps — all values passed explicitly as args

  // Single effect that owns all fetching. Fires whenever any filter changes.
  useEffect(() => {
    clearTimeout(searchTimeout.current);
    // Debounce only when search text changes; instant for page/status changes
    const delay = 0; // will be overridden by search input handler
    fetchBlogs({ pg: page, status: statusFilter, q: search });
    return () => clearTimeout(searchTimeout.current);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page, statusFilter]); // search has its own debounced path below

  // Debounced search — resets page to 1 and re-fetches
  const handleSearchChange = (val) => {
    setSearch(val);
    clearTimeout(searchTimeout.current);
    searchTimeout.current = setTimeout(() => {
      setPage(1); // triggers the effect above, but we also need to pass latest val
      fetchBlogs({ pg: 1, status: statusFilter, q: val });
    }, 350);
  };

  // When statusFilter changes, reset page too
  const handleStatusFilterChange = (val) => {
    setStatusFilter(val);
    setPage(1);
    setSelected([]); // clear selections when filter changes — they're page-specific
  };

  const handlePageChange = (p) => {
    setPage(p);
    setSelected([]); // clear selections on page turn
  };
  ...
  // ── Save ───────────────────────────────────────────────────────
  // overrideStatus lets "Save Draft" bypass the form's current status value
  // without triggering a setState + re-render race.
  ...
  // Extras tab label with count — computed inline so it's always current
      ...
      <div className="flex min-h-screen bg-slate-50">
        ...
        <div className="flex-1 ml-64 min-h-screen overflow-y-auto">
          <div className="p-6 lg:p-8">
            ...
            {/* Stat cards — server totals, independent of current filter */}
        ...
        <div
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl shadow-slate-900/20 overflow-hidden my-auto">
            ...
            <div className="p-6 max-h-[62vh] overflow-y-auto space-y-4">
              ...
              {activeTab === "content" && (<>
                ...
                <Field label="Excerpt" hint={`${form.excerpt.length}/300`} icon={AlignLeft}>
                  <textarea className={textareaCls} style={{ height: 76 }} value={form.excerpt}
                ...
                <Field label="Content" hint="HTML or Markdown" icon={FileText}>
                  <textarea className={`${textareaCls} font-mono text-xs`} style={{ height: 200 }} value={form.content}
              ...
              {activeTab === "seo" && (<>
                ...
                <Field label="Meta Description" hint={`${form.metaDescription.length}/160`} icon={AlignLeft}>
                  <textarea className={textareaCls} style={{ height: 80 }} value={form.metaDescription}
                    ...
                    {form.sections.length === 0
                      ...
                      : form.sections.map((sec, i) => (
                          <div key={i} className="bg-slate-50 border border-slate-100 rounded-xl p-4 mb-3 last:mb-0">
                            ...
                            <textarea className={textareaCls} style={{ height: 80 }} placeholder="Content" value={sec.content}
                    ...
                    {form.faqs.length === 0
                      ...
                      : form.faqs.map((faq, i) => (
                          <div key={i} className="bg-slate-50 border border-slate-100 rounded-xl p-4 mb-3 last:mb-0">
                            ...
                            <textarea className={textareaCls} style={{ height: 80 }} placeholder="Answer" value={faq.answer}
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 bg-slate-50/50">
              ...
              <div className="flex items-center gap-2">
                {/* Save Draft — available on both create and edit */}
                {form.status !== "draft" ? null : (
                  <button
                    onClick={() => handleSave("draft")}
                    disabled={saving}
                    className="px-4 py-2.5 text-sm font-semibold text-[#154895] bg-blue-50 border border-blue-100 rounded-xl hover:bg-blue-100 disabled:opacity-60 transition-colors">
                    Save Draft
                  </button>
                )}
                <button onClick={() => handleSave()} disabled={saving}
                  ...
                  {saving
                    ...
                    : <><Check className="w-4 h-4" /> {modal === "create" ? "Publish Post" : "Save Changes"}</>
                  }
                </button>
              </div>
            </div>
